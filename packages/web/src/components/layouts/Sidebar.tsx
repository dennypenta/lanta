    ...
    <aside class="w-64 flex-shrink-0 border-r border-border-dark bg-surface-dark flex flex-col justify-between h-full">
      <div class="flex flex-col gap-1 p-3">
        ...
        <div class="group flex items-center gap-3 px-2 py-3 mb-4 cursor-pointer hover:bg-white/5 rounded-md transition-colors">
          <div
            class="bg-center bg-no-repeat bg-cover rounded-md size-8 bg-indigo-500 flex items-center justify-center text-xs font-bold text-white shadow-inner"
            data-alt="Workspace logo abstract gradient"
            style="background-image: linear-gradient(135deg, #6366f1 0%, #a855f7 100%);"
          >
            AC
          </div>
        ...
        <div class="space-y-0.5">
          <MenuItem
            ...
            active={true}
          />
          <MenuItem icon={<FolderOpen class="size-4" />} text="Projects" />
        </div>
        {/* Section Divider  */}
        ...
        <div class="space-y-0.5">
          ...
          <MenuItem icon={<Inbox class="size-4" />} text="Inbox" />
          <MenuItem icon={<CircleUser class="size-4" />} text="My Issues" />
        </div>
        {/* Section Divider  */}
        <div class="h-px bg-border-dark my-2 mx-3"></div>
        {/* Teams Section  */}
        <div class="space-y-0.5">
          ...
          <MenuItem
            ...
            text="Engineering"
          />
          <MenuItem icon={<div class="size-2 rounded-full bg-purple-500"></div>} text="Design" />
        </div>
      </div>
      {/* Sidebar Footer  */}
      <div class="p-3 border-t border-border-dark">
        <MenuItem icon={<MessageCircleQuestionMark class="size-4" />} text="Help & Support" />
      </div>
    </aside>
  )
}

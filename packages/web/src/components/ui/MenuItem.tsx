  ...
  return (
    <div
      class={`flex items-center gap-2.5 px-3 py-1.5 rounded-md cursor-pointer transition-colors group ${
        local.active ? 'bg-white/5 text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'
      } ${local.class || ''}`}
    ...
    >
      ...
      {local.rightContent && (
        <span class="ml-auto text-xs text-gray-500 font-mono">{local.rightContent}</span>
      )}
    </div>
  )
}
